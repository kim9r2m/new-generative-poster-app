...
def add_color(name, r, g, b, filepath=PALETTE_FILE):
    ...
    new_color = pd.DataFrame([{"name":name,"r":float(r),"g":float(g),"b":float(b)}])
...
def load_palette_from_csv(filepath=PALETTE_FILE):
    df = read_csv_palette(filepath)
    if df.empty: return [(0.5,0.5,0.5)]
...
def extract_palette_from_image(img_bytes, num_colors=6):
    ...
    extracted = [{"name": f"ref_{i+1}", 
                  "r": c.rgb.r/255.0, 
                  "g": c.rgb.g/255.0, 
                  "b": c.rgb.b/255.0} for i,c in enumerate(colors)]
...
def generate_pastel_palette(k=5):
    return [colorsys.hsv_to_rgb(random.random(), random.uniform(0.15,0.35), random.uniform(0.9,1.0)) for _ in range(k)]

def generate_vivid_palette(k=5):
    return [colorsys.hsv_to_rgb(random.random(), random.uniform(0.8,1.0), random.uniform(0.8,1.0)) for _ in range(k)]

def generate_mono_palette(k=5, base_h=0.60):
    return [colorsys.hsv_to_rgb((base_h + random.uniform(-0.05,0.05))%1.0, random.uniform(0.2,0.8), random.uniform(0.5,1.0)) for _ in range(k)]

def generate_random_palette(k=5):
    return [(random.random(),random.random(),random.random()) for _ in range(k)]

def get_palette(mode, k=6):
    if mode == "csv": return load_palette_from_csv(PALETTE_FILE)
    if mode == "reference": return load_palette_from_csv(REFERENCE_PALETTE_FILE)
    if mode == "pastel": return generate_pastel_palette(k)
    if mode == "vivid": return generate_vivid_palette(k)
    if mode == "mono": return generate_mono_palette(k)
...
def draw_poster(n_layers, wobble, palette_mode, seed):
    ...
    for _ in range(n_layers):
        ...
        plt.fill(x, y, color=color, alpha=random.uniform(0.25, 0.6), edgecolor=(0,0,0,0))

    plt.text(0.05, 0.95, f"Generative Poster • {palette_mode}", fontsize=16, weight="bold", transform=plt.gca().transAxes)
...
palette_mode = st.sidebar.selectbox("Palette Mode", ["csv", "pastel", "vivid", "mono", "random", "reference"])
...
if st.button("🎨 Generate Poster"):
    try:
        plt_obj = draw_poster(layers, wobble, palette_mode, seed)
        st.pyplot(plt_obj)
